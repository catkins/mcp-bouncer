...
fn settings_and_tools_persist() {
    ...
    s.mcp_servers.push(MCPServerConfig{ name: "srv1".into(), description: "test".into(), transport: Some(TransportType::StreamableHttp), command: "".into(), args: None, env: None, endpoint: Some("http://127.0.0.1".into()), headers: None, requires_auth: Some(false), enabled: true });
