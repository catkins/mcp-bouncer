use mcp_bouncer::events::{EventEmitter, servers_updated, client_status_changed};
...
fn event_sequence_for_auth_lifecycle() {
    ...
    assert_eq!(events[0].0.as_str(), mcp_bouncer::events::EVENT_SERVERS_UPDATED);
    assert_eq!(events[1].0.as_str(), mcp_bouncer::events::EVENT_CLIENT_STATUS_CHANGED);
    assert_eq!(events[2].0.as_str(), mcp_bouncer::events::EVENT_CLIENT_STATUS_CHANGED);
    assert_eq!(events[3].0.as_str(), mcp_bouncer::events::EVENT_CLIENT_STATUS_CHANGED);
